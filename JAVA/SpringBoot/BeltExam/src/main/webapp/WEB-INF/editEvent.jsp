...
<body>
	<h1>Edit ${ event.name }</h1>
 	<form:form method="POST" action="/events/${id}/edit" modelAttribute="event">
		...
		<form:input path="datehosting" type="date" value="${event.datehosting}"/></form:label>
		<br>
		<p>Location</p>
		 <form:input path="cities" value="${event.cities}"/>
		 <form:select path="state" items="${USAstates}" value="${event.state}"/>
