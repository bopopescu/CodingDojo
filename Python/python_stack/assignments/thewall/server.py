...
def toregister():
    ...
    and request.form['password'] == request.form['passwordconfirm']):
        ...
        newusers = mysql.query_db(query, data)
        print newusers
        print newusers
        id = newusers
        session['id'] = id
...
def getregister():
    if 'id' in session:
        ...
        individualcomment = "select comments.messages_id, users.first_name, comments.comment, comments.users_id, users.id from users, comments where comments.users_id = users.id;"
        getcomments = mysql.query_db(individualcomment)
        individualmessage = "select messages.id, users.first_name, messages.message, messages.users_id from users, messages where messages.users_id = users.id;"
        getmessages = mysql.query_db(individualmessage)
        print "comment", getcomments
        print "message", getmessages
        return render_template('wall.html', first_name = first_name, userqueryinfo=userqueryinfo, last_name =  last_name, individualmessage=getmessages, individalcomment=getcomments, allmessages = allmessages, allcomments=allcomments)
    else:
        flash('fail!')
        return redirect('/')
